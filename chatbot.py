...
def convert_messages_to_gemini_prompt(chat_history: list[dict]) -> str:
    """
...
def get_chatbot_response_groq(chat_history: list[dict]) -> str:
    ...
    response = get_groq_client().chat.completions.create(
        model="llama-3.1-8b-instant", messages=chat_history, temperature=0.7
    )
...
def get_chatbot_response_genai(chat_history: list[dict]) -> str:
    ...
    1. Called the model models.generate_content()
        ...
        1.4 Return .text
    """
    prompt = convert_messages_to_gemini_prompt(chat_history)
...
def get_chatbot_response_anthropic(chat_history: list[dict]) -> str:
    ...
    """
    system_prompt, messages = convert_messages_to_anthropic_format(
        chat_history
    )
